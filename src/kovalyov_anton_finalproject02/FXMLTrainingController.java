...
public class FXMLTrainingController implements Initializable{
    ...
    protected void trainAndSaveDirectory() throws ClassNotFoundException, IOException {
        ...
        try {
            ...
            errorAlert("Directory doesn't exist");
        }
        catch (IndexOutOfBoundsException ex) {
            errorAlert("Directory contains no jpg images");
        }
        catch (IllegalArgumentException ex) {
            errorAlert("All images in the directory must be of equal size");
        }
    }
    /**
     * the method that creates an error alert when the user inputs a bad directory
     * in the set directory TextField
     */
    private void errorAlert(String s) {
        Alert alert = new Alert(AlertType.ERROR);
        alert.setTitle("Error");
        alert.setHeaderText("Bad Directory");
